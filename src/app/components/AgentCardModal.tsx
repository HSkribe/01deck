...
function CardFront({ agent, config }: { agent: Agent; config: typeof rarityConfig[keyof typeof rarityConfig] }) {
  const { currentTheme: t, isPluginEnabled } = useApp();
  ...
  return (
    <div className="relative w-full h-full flex flex-col overflow-hidden rounded-2xl">
      ...
      {/* Portrait */}
      <div className="relative z-10 flex-1 mx-5 mb-4 rounded-xl overflow-hidden"
        style={{
      ...
      <div className="relative z-10 px-5 pb-5">
        <div className="flex items-end justify-between mb-2">
          ...
          <RarityBadge rarity={agent.rarity} rarityCount={agent.rarityCount} size="md" />
        </div>
        <div
          className="inline-flex items-center gap-1.5 mb-2 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-[0.18em]"
          style={{
            background: verificationTone.background,
            border: `1px solid ${verificationTone.border}`,
            color: verificationTone.color,
          }}
        >
          <VerificationIcon size={11} />
          <span>{verificationTone.label}</span>
        </div>
        {evolutionPluginEnabled && agent.hasEvolution ? (
          <div
            className="inline-flex items-center gap-1.5 mb-2 ml-2 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-[0.18em]"
            style={{
    ...
    <div className="relative w-full h-full flex flex-col overflow-hidden rounded-2xl">
      ...
      <div className="relative z-10 flex flex-col h-full p-5 overflow-y-auto">
        ...
        {evolutionPluginEnabled && agent.hasEvolution ? (
          <div className="mb-4">
            <div className="flex items-center gap-1 mb-1.5">
              <Dna size={11} style={{ color: '#34d399' }} />
              <span className="text-xs uppercase tracking-wider" style={{ color: '#86efac' }}>Evolution Profile</span>
            </div>
            <div className="text-xs mb-2" style={{ color: 'rgba(255,255,255,0.6)' }}>
              Current stage: {agent.evolutionStage ?? 0} · {(agent.evolutionTraits ?? []).length} active traits
            </div>
            <div className="flex flex-wrap gap-1">
              {(agent.evolutionTraits ?? []).map(trait => (
                <span
                  key={trait}
                  className="text-xs px-1.5 py-0.5 rounded"
                  style={{
