...
export function ChatWindow() {
  const {
    ...
    backendSessionStatus, backendAuthEnabled, authenticateBackend, logoutBackend,
    isChatStreaming,
    currentTheme: t,
  } = useApp();
          ...
          >
            ...
            <div className="flex items-center gap-1 ml-2">
              <motion.button
                ...
                style={{ color: backendSessionStatus === 'authenticated' ? t.accent : t.textMuted }}
                whileHover={{ color: t.text, scale: 1.1 }}
                title={backendSessionStatus === 'authenticated' ? 'Backend chat connected' : 'Connect beta access for server-backed chat'}
              >
                <KeyRound size={12} />
              </motion.button>
              <motion.button
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{
                        color: backendSessionStatus === 'authenticated' ? t.text : t.textMuted,
                    ...
                    >
                      {backendAuthEnabled
                        ? backendSessionStatus === 'authenticated'
                          ? 'Beta session active'
                          : backendSessionStatus === 'checking'
                            ? 'Checking beta session...'
                            : 'Beta session required'
                        : 'Backend auth disabled for this environment'}
                    </div>
                    {backendSessionStatus === 'authenticated' ? (
                      <button
                        type="button"
                  ...
                  {backendAuthEnabled && backendSessionStatus !== 'authenticated' ? (
