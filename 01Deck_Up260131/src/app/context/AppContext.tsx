...
interface AppContextType {
  ...
  hubInitialView: string | null;
  setHubInitialView: (v: string | null) => void;

  // Evolution Lab
  showEvolutionLab: boolean;
  setShowEvolutionLab: (v: boolean) => void;

  // Page context (for dynamic TopBar left side)
  pageContext: { title: string; subtitle: string };
  setPageContext: (ctx: { title: string; subtitle: string }) => void;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [hubInitialView, setHubInitialView] = useState<string | null>(null);
  const [showEvolutionLab, setShowEvolutionLab] = useState(false);
  const [pageContext, setPageContext] = useState({ title: 'AGENT VIEWER', subtitle: '01Deck Active Protocol' });
