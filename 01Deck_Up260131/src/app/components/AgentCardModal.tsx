...
import { X, Clock, Wifi, WifiOff, Tag, MessageSquare, ChevronRight, AlertCircle, Dna } from 'lucide-react';
...
// ─── Holographic Evolution Reveal ────────────────────────
function EvolutionReveal({ agent, onDone }: { agent: Agent; onDone: () => void }) {
  const stageLabels = ['Base', 'Awakened', 'Ascended', 'Transcendent'];
  const stage = agent.evolutionStage ?? 0;
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 z-30 rounded-2xl overflow-hidden flex flex-col items-center justify-center"
      style={{ background: 'rgba(4,4,12,0.92)', backdropFilter: 'blur(4px)' }}
    >
      {/* Rotating prism rings */}
      {[0, 1, 2].map(i => (
        <motion.div
          key={i}
          ...
          style={{
            ...
            background: `conic-gradient(from ${i * 120}deg, #ff0080, #ff8c00, #ffed00, #00ff88, #00cfff, #a855f7, #ff0080) border-box`,
      ...
      {/* Center agent initial */}
      <motion.div
        initial={{ scale: 0.4, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.2, type: 'spring', stiffness: 180, damping: 12 }}
        className="relative z-10 w-16 h-16 rounded-2xl overflow-hidden mb-4"
        style={{ boxShadow: '0 0 32px rgba(168,85,247,0.8), 0 0 64px rgba(6,182,212,0.4)' }}
      >
        <img src={agent.portrait} alt="" className="w-full h-full object-cover" />
        {/* Rainbow overlay */}
        <motion.div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, rgba(255,0,128,0.3), rgba(168,85,247,0.3), rgba(6,182,212,0.3), rgba(0,255,136,0.3))' }}
      ...
      >
        <div className="flex items-center gap-2 justify-center mb-1">
          <Dna size={12} style={{ color: '#10b981' }} />
          <span className="text-[10px] uppercase tracking-[0.25em]" style={{ color: '#10b981' }}>
            Evolution Agent
          </span>
          <Dna size={12} style={{ color: '#10b981' }} />
        </div>
        <motion.p
          ...
          style={{ background: 'linear-gradient(90deg, #ff0080, #a855f7, #00cfff, #00ff88)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundSize: '200% 100%' }}
      ...
      {/* Full-card prismatic sweep */}
      <motion.div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'linear-gradient(105deg, transparent 20%, rgba(255,0,128,0.06) 35%, rgba(168,85,247,0.1) 45%, rgba(6,182,212,0.1) 55%, rgba(0,255,136,0.06) 65%, transparent 80%)',
        }}
        animate={{ x: ['-100%', '150%'] }}
...
function CardBack({ agent, config }: { agent: Agent; config: typeof rarityConfig[keyof typeof rarityConfig] }) {
  const { currentTheme: t, setChatAgent, setIsChatOpen } = useApp();
  ...
  return (
    <div className="relative w-full h-full flex flex-col overflow-hidden rounded-2xl">
      ...
      <div className="relative z-10 flex flex-col h-full p-5 overflow-y-auto">
        ...
        {/* CTA */}
        <motion.button
          onClick={() => { setChatAgent(agent); setIsChatOpen(true); }}
...
export function AgentCardModal() {
  ...
  const [showEvoReveal, setShowEvoReveal] = useState(false);

  useEffect(() => {
    if (!isCardVisible) { setIsFlipped(false); setShowEvoReveal(false); }
    // Trigger evolution reveal when card opens for an evolution agent
    if (isCardVisible && activeAgent?.hasEvolution) {
      setShowEvoReveal(true);
    }
  }, [isCardVisible, activeAgent]);
    ...
    <AnimatePresence>
      {isCardVisible && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
            ...
            >
              ...
              <motion.div
                ...
                style={{ perspective: 1200, cursor: showEvoReveal ? 'default' : 'pointer' }}
                onClick={() => !showEvoReveal && setIsFlipped(f => !f)}
              >
                <motion.div
              ...
              <AnimatePresence>
                {showEvoReveal && activeAgent.hasEvolution && (
                  <EvolutionReveal agent={activeAgent} onDone={() => setShowEvoReveal(false)} />
                )}
              </AnimatePresence>
              ...
              <motion.div
                ...
                style={{ background: `radial-gradient(ellipse, ${activeAgent.hasEvolution && !showEvoReveal ? 'rgba(168,85,247,0.45)' : config.glowColor}, transparent 70%)` }}
          ...
          <RarityInfoModal agent={activeAgent} isOpen={showRarityInfo} onClose={() => setShowRarityInfo(false)} />
        </>
      )}
    </AnimatePresence>
  );
